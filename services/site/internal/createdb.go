...
func Createdb() {
	...
	// create table
	scriptSQL_forum_table, err := os.ReadFile("../database/script_create_table_forum.sql")
	CheckErr(err, "3- open file script_create_table_forum.sql (Createdb.go)")
	_, err = database.Exec(string(scriptSQL_forum_table))
	CheckErr(err, "3- Err execute script_create_table_forum in db (Createdb.go)")
	...
	if idTypeUnit == sql.ErrNoRows {
		...
		CheckErr(err, "4- open file script_INSERT_ListTypeUnit.sql (Createdb.go)")
		_, err = database.Exec(string(scriptSQL_type))
		CheckErr(err, "4- Err script Insert ListTypeUnit in db (Createdb.go)")
	}
	...
	if idClass == sql.ErrNoRows {
		...
		CheckErr(err, "5- open file script_INSERT_ListGameCharacter.sql (Createdb.go)")
		_, err = database.Exec(string(scriptSQL_class))
		CheckErr(err, "5- Err script Insert ListGameCharacter in db (Createdb.go)")
	}
	...
	if idbaseUnit == sql.ErrNoRows {
		...
		CheckErr(err, "6- open file script_INSERT_ListUnit.sql (Createdb.go)")
		_, err = database.Exec(string(scriptSQL_unit))
		CheckErr(err, "6- Err script Insert ListUnit in db (Createdb.go)")
	}
